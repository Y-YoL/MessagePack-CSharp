    ...
    ImmutableHashSet<string>? AdditionalAllowTypes = null)
{
    ...
    {
        var config = additionalTexts.FirstOrDefault(x => string.Equals(Path.GetFileName(x.Path), JsonAdditionalAllowTypesOptionsFileName, StringComparison.OrdinalIgnoreCase));
        if (config is null)
        {
